...
alzheimers %>%
  mutate(male=factor(male),
  Genotype = fct_infreq(fct_lump(Genotype,n=3))) %>%# now we can combine three of the Genotypes into one by using lump. the n  is the one we don't want to lump, aka the ones not in the first 3. by using the fct_infreq, we use the default variable value as the most common one, otherwise the default will be the one first in the alphabet. 
  count(Genotype)
```
#Sampling
...
#Data manipulation
By using recipe, we make sure the preprocessing manipulation steps works for future data. Look into the step_ functions in the recipe package to get help in preprocessing. 
```{r}
# don't touch the variable diagnosis, since it's the one we want to predict. this will be saved so that no other recipes over-write it. 
alz_train %>%
  recipe(diagnosis ~ .) -> 
  alz_preprocess
...
alz_preprocess %>%
  ...
  alz_preprocess_scale # save as separate to be able to test your model with different preprocessing
alz_preprocess # to look at what the preprocessing step is doing

prep(alz_preprocess_scale, training=alz_train)
